...
class LightrailAPICall
{
    ...
    private static function handleError($httpCode, $response)
    {
        ...
        switch ($httpCode) {
            case 400:
                throw new Exceptions\BadParameterException($message);
            case 401:
            case 403:
                throw new Exceptions\AuthorizationException($message);
            case 404:
                throw new Exceptions\ObjectNotFoundException($message);
            default:
                throw new Exceptions\LightrailException($message);
        }
    }
